...
public class ForgeMixinConfigPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (mixinClassName.equals("io.github.alexcode10010.mediabellum.forge.mixin.MixinDatagenMain")) {
            var shouldApply = System.getProperty("mediabellum.apply-datagen-mixin", "false").equals("true");
            if (shouldApply) {
                Mediabellum.LOGGER.warn("Applying scuffed datagen mixin. This should not happen if not running datagen!");
            }
            return shouldApply;
        }
        return true;
